...
interface MultiSelectProps
   ...
   * Called when user clicks Close button or clicks outside the popover.
   * Optional.
   */
  onClose?: () => void

  /**
   * Callback function triggered when clicking outside the component.
   * Similar to onBlur for input elements, useful for closing edit mode.
   * Optional.
   */
  onBlur?: () => void
}
...
export function MultiSelect({
  ...
  onBlur,
  ...props
}: MultiSelectProps & { ref?: React.RefObject<MultiSelectRef | null> }) {
  ...
  const toggleOption = (optionValue: string) => {
    ...
    if (option?.disabled)
      return
    const newSelectedValues = selectedValues.includes(optionValue) ? selectedValues.filter(value => value !== optionValue) : [...selectedValues, optionValue]
    setSelectedValues(newSelectedValues)
    onValueChange(newSelectedValues)
    if (closeOnSelect) {
      setIsPopoverOpen(false)
  ...
  React.useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // 检查是否点击在 Popover 内容中
      const isClickInsidePopover = (target as Element).closest('[role="listbox"], [data-radix-popper-content-wrapper]')
    ...
    <>
      ...
      <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
        ...
        <div id={selectedCountId} className="sr-only" aria-live="polite">
          {selectedValues.length === 0 ? 'No options selected' : `${selectedValues.length} option${selectedValues.length === 1 ? '' : 's'} selected: ${selectedValues
